...
export function loadWithRetry<S, T>(
  ...
  return source.switchMap(key => {
    const sideChannel = new Subject<ILoadResult<T>>();
    let attempt = 0;
    return Observable.merge(
      ...
      Observable.defer(() => {
        ...
        .retryWhen(errors => errors
          ...
          .do(_ => sideChannel.next({ status: ILoadResultStatus.Waiting }))
          .delayWhen(() => retryDelay(options, attempt))
          .do(_ => sideChannel.next({ status: ILoadResultStatus.Retrying }))
        )
